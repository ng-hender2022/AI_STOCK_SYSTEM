...
from .signal_logic import OBVSignalLogic, OBVOutput
...
class OBVExpertWriter:
    """
    End-to-end V4OBV pipeline.

    Usage:
        writer = OBVExpertWriter(market_db, signals_db)
    ...
    def __init__(self, market_db: str | Path, signals_db: str | Path):
        ...
        self.feature_builder = OBVFeatureBuilder(market_db)
        self.signal_logic = OBVSignalLogic()
    ...
    def _write_output(self, conn: sqlite3.Connection, output: OBVOutput, features) -> None:
        metadata = {
            "obv_slope_norm": round(features.obv_slope_norm, 6),
            "obv_divergence": features.obv_divergence,
            "obv_new_high": features.obv_new_high,
            "obv_new_low": features.obv_new_low,
            "obv_confirms_price": features.obv_confirms_price,
            "obv_norm": output.obv_norm,
        ...
        conn.execute(
            ...
            (
                ...
                output.obv_score, output.obv_norm,
                output.signal_code, output.signal_quality,
                json.dumps(metadata),
            ),
        )

    def run_symbol(self, symbol: str, target_date: str) -> OBVOutput:
