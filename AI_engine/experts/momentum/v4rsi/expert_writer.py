...
from .signal_logic import RSISignalLogic, RSIOutput
...
class RSIExpertWriter:
    """
    End-to-end V4RSI pipeline.

    Usage:
        writer = RSIExpertWriter(market_db, signals_db)
    ...
    def __init__(self, market_db: str | Path, signals_db: str | Path):
        ...
        self.feature_builder = RSIFeatureBuilder(market_db)
        self.signal_logic = RSISignalLogic()
    ...
    def _write_output(self, conn: sqlite3.Connection, output: RSIOutput) -> None:
        metadata = {
            ...
            "rsi_slope": round(output.rsi_slope, 6),
            "rsi_ma10": round(output.rsi_ma10, 4),
            "rsi_above_50": output.rsi_above_50,
            "rsi_zone": output.rsi_zone,
    ...
    def run_symbol(self, symbol: str, target_date: str) -> RSIOutput:
