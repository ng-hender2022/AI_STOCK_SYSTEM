...
from .signal_logic import STOSignalLogic, STOOutput
...
class STOExpertWriter:
    """
    End-to-end V4STO pipeline.

    Usage:
        writer = STOExpertWriter(market_db, signals_db)
    ...
    def __init__(self, market_db: str | Path, signals_db: str | Path):
        ...
        self.feature_builder = STOFeatureBuilder(market_db)
        self.signal_logic = STOSignalLogic()
    ...
    def _write_output(self, conn: sqlite3.Connection, output: STOOutput) -> None:
        metadata = {
            "stoch_k": round(output.stoch_k, 4),
            "stoch_d": round(output.stoch_d, 4),
            "stoch_k_slope": round(output.stoch_k_slope, 6),
            "k_above_d": int(output.k_above_d),
            "stoch_zone": int(output.stoch_zone),
            "stoch_divergence": int(output.stoch_divergence),
            "stoch_cross_in_zone": int(output.stoch_cross_in_zone),
            "sto_norm": round(output.sto_norm, 4),
            "signal_quality": int(output.signal_quality),
        }
    ...
    def run_symbol(self, symbol: str, target_date: str) -> STOOutput:
