...
from .signal_logic import ADXSignalLogic, ADXOutput
...
class ADXExpertWriter:
    """
    End-to-end V4ADX pipeline.

    Usage:
        writer = ADXExpertWriter(market_db, signals_db)
    ...
    def __init__(self, market_db: str | Path, signals_db: str | Path):
        ...
        self.feature_builder = ADXFeatureBuilder(market_db)
        self.signal_logic = ADXSignalLogic()
    ...
    def _write_output(self, conn: sqlite3.Connection, output: ADXOutput, features) -> None:
        metadata = {
            "adx_value": round(features.adx_value, 4),
            "plus_di": round(features.plus_di, 4),
            "minus_di": round(features.minus_di, 4),
            "di_diff": round(features.di_diff, 4),
            "adx_slope": round(features.adx_slope, 4),
            "adx_rising": bool(features.adx_rising),
            "di_cross_bull": bool(features.di_cross_bull),
            "di_cross_bear": bool(features.di_cross_bear),
            "adx_score": features.adx_score,
            "di_score": round(features.di_score, 4),
        }
        ...
        conn.execute(
            ...
            (
                ...
                output.primary_score, output.secondary_score,
                output.signal_code, output.signal_quality,
                json.dumps(metadata),
            ),
        )

    def run_symbol(self, symbol: str, target_date: str) -> ADXOutput:
