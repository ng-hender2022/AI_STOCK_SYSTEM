"""
V4TREND_PATTERN Expert Writer
Ghi output vao signals.db -> expert_signals.
Doc universe tu MASTER_UNIVERSE.md.
"""

import json
import re
import sqlite3
from pathlib import Path

from .feature_builder import TPFeatureBuilder
from .signal_logic import TPSignalLogic, TPOutput

MASTER_UNIVERSE_PATH = Path(r"D:\AI\AI_brain\SYSTEM\MASTER_UNIVERSE.md")


def load_universe() -> list[str]:
    text = MASTER_UNIVERSE_PATH.read_text(encoding="utf-8")
    match = re.search(r"## DANH SACH DAY DU.*?```\s*\n(.*?)```", text, re.DOTALL)
    if not match:
        match = re.search(r"## DANH S.*?```\s*\n(.*?)```", text, re.DOTALL)
    if not match:
        raise RuntimeError(f"Cannot parse universe from {MASTER_UNIVERSE_PATH}")
    raw = match.group(1)
    symbols = [s.strip() for s in raw.replace("\n", ",").split(",") if s.strip()]
    return sorted(symbols)


class TPExpertWriter:
    """
    End-to-end V4TREND_PATTERN pipeline.

    Usage:
        writer = TPExpertWriter(market_db, signals_db)
        output = writer.run_symbol("FPT", "2026-03-16")
        results = writer.run_all("2026-03-16")
    """

    EXPERT_ID = "V4TREND_PATTERN"

    def __init__(self, market_db: str | Path, signals_db: str | Path):
        self.market_db = str(market_db)
        self.signals_db = str(signals_db)
        self.feature_builder = TPFeatureBuilder(market_db)
        self.signal_logic = TPSignalLogic()

    def _connect_signals(self) -> sqlite3.Connection:
        conn = sqlite3.connect(self.signals_db, timeout=30)
        conn.execute("PRAGMA journal_mode=WAL")
        conn.execute("PRAGMA foreign_keys=ON")
        return conn

    def _write_output(self, conn: sqlite3.Connection, output: TPOutput, features) -> None:
        pattern = features.pattern
        metadata = {
            "pattern_type": pattern.pattern_type if pattern else "none",
            "pattern_direction": pattern.pattern_direction if pattern else "neutral",
            "confirmed": bool(pattern.confirmed) if pattern else False,
            "target_pct": round(float(pattern.target_pct), 6) if pattern else 0.0,
            "breakout_volume_ratio": round(float(pattern.breakout_volume_ratio), 4) if pattern else 0.0,
            "pattern_duration": int(pattern.pattern_duration) if pattern else 0,
            "pattern_failure": bool(pattern.pattern_failure) if pattern else False,
            "pattern_norm": round(float(output.pattern_norm), 6),
            "base_pattern_score": round(float(output.base_pattern_score), 4),
            "confirmation_score": round(float(output.confirmation_score), 4),
            "target_score": round(float(output.target_score), 4),
        }

        conn.execute(
            """INSERT OR REPLACE INTO expert_signals
               (symbol, date, snapshot_time, expert_id,
                primary_score, secondary_score,
                signal_code, signal_quality, metadata_json)
               VALUES (?, ?, 'EOD', ?, ?, ?, ?, ?, ?)""",
            (
                output.symbol, output.date, self.EXPERT_ID,
                output.pattern_score, output.pattern_norm,
                output.signal_code, output.signal_quality,
                json.dumps(metadata),
            ),
        )

    def run_symbol(self, symbol: str, target_date: str) -> TPOutput:
        features = self.feature_builder.build(symbol, target_date)
        output = self.signal_logic.compute(features)
        conn = self._connect_signals()
        try:
            if output.has_sufficient_data:
                self._write_output(conn, output, features)
            conn.commit()
        finally:
            conn.close()
        return output

    def run_all(
        self, target_date: str, symbols: list[str] | None = None
    ) -> list[TPOutput]:
        if symbols is None:
            symbols = load_universe()
        features_list = self.feature_builder.build_batch(symbols, target_date)
        results = []
        conn = self._connect_signals()
        try:
            for feat in features_list:
                output = self.signal_logic.compute(feat)
                if output.has_sufficient_data:
                    self._write_output(conn, output, feat)
                results.append(output)
            conn.commit()
        finally:
            conn.close()
        return results
